...
if __name__ == "__main__":
    ...
    if args.iid:
        ...
        part_df = pd.read_csv(csv_file, header=1)
    # end
    frequence = [[] for i in range(args.num_users)]
    for i in range(args.num_users):
        frequence[i] = [part_df.iloc[i, j] for j in range(1, 11)]
    # frequence = [part_df.iloc[0, i] for i in range(1, 11)]
    validation_idx = [mnist_one_client_from_designed_frequence(dataset_test, frequence[i], 1000) for i in
    ...
    for iter in range(args.epochs):
        ...
        for i in range(args.num_users):
            ...
            _, test_loss = brca_test(net=copy.deepcopy(net).to(args.device), w=w,
                                     dataset=dataset_test, args=args, idx=validation_idx[i])
    ...
    filename = f"同分布验证集_seed{args.seed}_alpha{args.dir_alpha}_iid{args.iid}_users{args.num_users}.txt"
